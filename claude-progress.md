...
- **Session 023 (improve audit)**: Full /improve audit across correctness, security, performance, tests, tech-debt. 4 parallel subagents returned 39 findings. Vetted and prioritized 20 into 14 implementation plans (009-022). Top bugs: due reminder dedup per-user (CORRECT-01), cancelDelayed doesn't cancel BullMQ (CORRECT-02), instant emails marked SENT before delivery (CORRECT-03). Security: no file allowlist (ATTACH-01), no security headers (HDR-01), rate-limit IP collapse (RATE-01). Plans written to plans/009-022.md. Base commit: 732acb4.
