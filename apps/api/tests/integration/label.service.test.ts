...
import { ConflictError, BadRequestError, ForbiddenError, NotFoundError } from '../../src/shared/errors';
...
describe('label.service', () => {
  ...
  it('cross-workspace assign throws TASK_LABEL_CROSS_WORKSPACE', async () => {
    ...
    const labelInW2 = await svc.createLabel(prisma, ownerId, w2.id, { name: 'foreign', color: 'red' });
    await expect(
      svc.assignToTask(prisma, ownerId, taskId, [labelInW2.id]),
    ).rejects.toMatchObject({ code: 'TASK_LABEL_CROSS_WORKSPACE' });
  });
  ...
  it('assignToTask replaces all + dual-writes labelsDeprecated', async () => {
    ...
    const assignments = await prisma.taskLabelAssignment.findMany({ where: { taskId, deletedAt: null } });
  ...
  it('cascade: deleting label removes assignments', async () => {
    ...
    const assignments = await prisma.taskLabelAssignment.findMany({ where: { labelId, deletedAt: null } });
  ...
  it('in-use label delete blocked', async () => {
    await svc.assignToTask(prisma, ownerId, taskId, [labelId]);
    await expect(
      svc.deleteLabel(prisma, ownerId, wid, labelId),
