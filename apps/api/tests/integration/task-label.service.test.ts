...
describe('task-label.service', () => {
  ...
  beforeEach(async () => {
    ...
    const l = await prisma.taskLabel.create({
      data: { workspaceId: wid, name: 'bug', color: 'red' },
    });
    labelId = l.id;
    const l2 = await prisma.taskLabel.create({
      data: { workspaceId: w2id, name: 'foreign', color: 'blue' },
    });
    w2LabelId = l2.id;
  });
  ...
  it('cross-workspace label assign → NotFoundError', async () => {
    await expect(taskLabelService.assign(wid, taskId, w2LabelId, ownerId)).rejects.toThrow(
      NotFoundError,
    );
  });

  it('non-member assign rejected', async () => {
    await expect(taskLabelService.assign(wid, taskId, labelId, outsiderId)).rejects.toThrow();
  });
