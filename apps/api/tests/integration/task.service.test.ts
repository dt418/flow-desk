...
describe('task.service', () => {
  ...
  beforeEach(async () => {
    ...
    colDone = await prisma.column.create({ data: { workspaceId: wid, name: 'Done', position: 1, isDoneColumn: true } });
  ...
  describe('list', () => {
    it('returns paginated tasks for member (cursor)', async () => {
      await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId } });
    ...
    it('filters by status, priority, search', async () => {
      await prisma.task.create({
        data: { workspaceId: wid, columnId: colTodo.id, title: 'unique-findme', createdById: ownerId, priority: 'HIGH', status: 'IN_PROGRESS', assigneeId: memberId },
      });
      const bySearch = await taskService.list({ workspaceId: wid, search: 'findme', limit: 10 } as never, ownerId);
      expect(bySearch.data.length).toBe(1);
      const byStatus = await taskService.list({ workspaceId: wid, status: 'IN_PROGRESS', limit: 10 } as never, ownerId);
      expect(byStatus.data.length).toBe(1);
    });

    it('non-member rejected', async () => {
      await expect(taskService.list({ workspaceId: wid, limit: 10 } as never, outsiderId)).rejects.toThrow(BadRequestError);
    });

    it('limit=1 + cursor pagination across 3 tasks', async () => {
      await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId } });
      await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'C', createdById: ownerId } });
      const p1 = await taskService.list({ workspaceId: wid, limit: 1 } as never, ownerId);
      expect(p1.data.length).toBe(1);
      expect(p1.nextCursor).not.toBeNull();
      const p2 = await taskService.list({ workspaceId: wid, limit: 1, cursor: p1.nextCursor! } as never, ownerId);
  ...
  describe('create', () => {
    it('creates with auto position', async () => {
      const t = await taskService.create(ownerId, {
        workspaceId: wid, columnId: colTodo.id, title: 'New', priority: 'MEDIUM', status: 'TODO',
      });
    ...
    it('parent honoured', async () => {
      const child = await taskService.create(ownerId, {
        workspaceId: wid, columnId: colTodo.id, title: 'child', priority: 'MEDIUM', status: 'TODO', parentTaskId: taskId,
      });
    ...
    it('non-member rejected', async () => {
      await expect(
        taskService.create(outsiderId, { workspaceId: wid, columnId: colTodo.id, title: 'x', priority: 'MEDIUM', status: 'TODO' }),
  ...
  describe('update', () => {
    ...
    it('404 for missing', async () => {
      await expect(taskService.update(ownerId, 'missing', { title: 'x' })).rejects.toThrow(NotFoundError);
    });

    it('non-member rejected', async () => {
      await expect(taskService.update(outsiderId, taskId, { title: 'x' })).rejects.toThrow(BadRequestError);
    });
  });
  ...
  describe('move', () => {
    it('to done column sets status DONE', async () => {
      const moved = await taskService.move(ownerId, taskId, { columnId: colDone.id, position: 0, version: 0 });
    ...
    it('reorder within same column', async () => {
      const tA = await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'A', createdById: ownerId, position: 1 } });
      await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'C', createdById: ownerId, position: 2 } });
      const moved = await taskService.move(ownerId, tA.id, { columnId: colTodo.id, position: 2, version: 0 });
  ...
  describe('createSubtask', () => {
    it('happy', async () => {
      const sub = await taskService.createSubtask(ownerId, taskId, {
        workspaceId: wid, columnId: colTodo.id, title: 'sub', priority: 'LOW', status: 'TODO',
      });
    ...
    it('parent 404', async () => {
      await expect(
        taskService.createSubtask(ownerId, 'missing', { workspaceId: wid, columnId: colTodo.id, title: 'sub', priority: 'LOW', status: 'TODO' }),
  ...
  describe('createDependency', () => {
    it('happy', async () => {
      const t2 = await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId } });
      const dep = await taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: t2.id });
      expect(dep.blockingTaskId).toBe(taskId);
    });

    it('self-block (400)', async () => {
      await expect(taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: taskId })).rejects.toThrow(BadRequestError);
    });

    it('missing task (404)', async () => {
      await expect(taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: 'missing' })).rejects.toThrow(NotFoundError);
    });
    ...
    it('cross-workspace (400)', async () => {
      ...
      await expect(taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: t2.id })).rejects.toThrow(BadRequestError);
    });

    it('duplicate (409)', async () => {
      const t2 = await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId } });
      await taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: t2.id });
      await expect(taskService.createDependency(ownerId, { blockingTaskId: taskId, blockedTaskId: t2.id })).rejects.toThrow(ConflictError);
    });

    it('cycle (400)', async () => {
      const tA = await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'A', createdById: ownerId } });
      const tB = await prisma.task.create({ data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId } });
      await taskService.createDependency(ownerId, { blockingTaskId: tA.id, blockedTaskId: tB.id });
      await expect(taskService.createDependency(ownerId, { blockingTaskId: tB.id, blockedTaskId: tA.id })).rejects.toThrow(BadRequestError);
    });
  });
