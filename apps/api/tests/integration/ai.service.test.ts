...
describe('ai.service', () => {
  ...
  describe('suggestAssignee', () => {
    ...
    it('non-member rejected', async () => {
      await expect(svc.suggestAssignee(db, outsiderId, { workspaceId: wid, title: 'x' })).rejects.toThrow(BadRequestError);
    });

    it('missing task (404)', async () => {
      await expect(svc.suggestAssignee(db, ownerId, { workspaceId: wid, taskId: 'missing' })).rejects.toThrow(NotFoundError);
    });

    it('no title or taskId (400)', async () => {
      await expect(svc.suggestAssignee(db, ownerId, { workspaceId: wid })).rejects.toThrow(BadRequestError);
    });
  });
  ...
  describe('autoSchedule', () => {
    it('schedules tasks respecting deps', async () => {
      const tA = await prisma.task.create({
        data: { workspaceId: wid, columnId: colTodo.id, title: 'A', createdById: ownerId, dueDate: new Date(Date.now() + 5 * 86400000) },
      });
      const tB = await prisma.task.create({
        data: { workspaceId: wid, columnId: colTodo.id, title: 'B', createdById: ownerId, dueDate: new Date(Date.now() + 10 * 86400000) },
      });
    ...
    it('detects cycle (400)', async () => {
      ...
      await expect(svc.autoSchedule(db, ownerId, { workspaceId: wid })).rejects.toThrow(BadRequestError);
    });

    it('non-member rejected', async () => {
      await expect(svc.autoSchedule(db, outsiderId, { workspaceId: wid })).rejects.toThrow(BadRequestError);
    });
  });
});